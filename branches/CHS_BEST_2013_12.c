...
{
	if(motor[servoMotor] - servoValue > tolerance)
		{
 ...
 while(true)
  {
  	...
  	if(servoCount == servoCountMax)
  		{
  			idleServo (servo1, servoValue1, 3);
  			idleServo (servo2, servoValue2, 3);
  			idleServo (servo3, servoValue3, 3);
  			idleServo (servo4, servoValue4, 3);
  			servoCount = 0;
  		}
   ...
   //joystick ch1 controls servo4
    ...
    if((vexRT[Ch2] <= -40) && (vexRT[Ch2] > -80))
		...
		if(vexRT[Ch3] == 0 && BtnX != 256 && BtnX != 512 && BtnX != 1024 && BtnX != 2048)
			{
				if((SensorValue[pot2] - tempSensor) > 5)
			  	{
			  		motor[motor1] = -30;
			  	}
			  if((SensorValue[pot2] - tempSensor) < -5)
